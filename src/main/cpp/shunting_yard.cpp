...
string expression_to_string(Expression exp,
                            function<string(exp_unit &)> get_func_call);
string expression_to_string(Expression exp);
...
public:
  ...
  FuncCall(const string &fname, const vector<Expression> &fargs)
...
public:
  ...
  exp_unit(int val, char op, FuncCall *func_call, string var, exp_unit_type t)
  ...
  string toString(function<string(exp_unit &)> get_func_call = [](exp_unit &e) {
  ...
  ~exp_unit() { delete _func_call; }
};

const map<char, int> exp_unit ::operator_precedence_map{
    {'^', 4}, {'*', 3}, {'/', 3}, {'+', 2}, {'-', 2}, {'(', 1}};
...
string FuncCall ::toCppString(vector<string> free_vars) {
  if (func_name == "Sum") {
    ...
    for (int i{0}; i < func_args.size(); i++) {
      if (i != 0)
        cpp_exp << ',';
      cpp_exp << expression_to_string(
          func_args.at(i), [free_vars](exp_unit &e) {
            if (e.func().func_name == "Binomial" ||
...
Expression parseExpression(string exp_str) {
  ...
  for (unsigned i{0}; i < exp_str.size(); i++) {
    ...
    if (exp_str.at(i) > 47 && exp_str.at(i) < 58) {
      unsigned j = i;
      for (j; j < exp_str.size() && exp_str.at(j) > 47 && exp_str.at(j) < 58;
           j++)
        ;
      exp.push_back({stoi(exp_str.substr(i, j - i))});
      i = j - 1;
    } else if ((exp_str.at(i) > 64 && exp_str.at(i) < 91) ||
               (exp_str.at(i) > 96 && exp_str.at(i) < 123)) {
      ...
      for (j;
           ...
           j++) {
        if (num_open_brack == 0 &&
            !(exp_str.at(j) > 64 && exp_str.at(j) < 91) &&
            !(exp_str.at(j) > 96 && exp_str.at(j) < 123) &&
            !(exp_str.at(j) > 47 && exp_str.at(j) < 58) &&
            exp_str.at(j) != '[') {
          isVar = true;
          break;
        }
      ...
      if (isVar) {
        ...
        i = j;
               ...
               (exp.size() == 0 ||
                exp.at(exp.size() - 1).type == exp_unit_type ::Operator)) {
      // unary minus
      unsigned j = i + 1;
      for (j; j < exp_str.size() && exp_str.at(j) > 47 && exp_str.at(j) < 58;
           j++)
...
int evaluateExpression(Expression exp, function<int(exp_unit)> get_var_val,
                       function<int(exp_unit)> get_func_val) {
...
string get_function_signature(exp_unit &e, string func_ret_pref = "int",
...
string generate_function_def(string eqn) {
  ...
  for (unsigned i{0}; i < max_sub_vec.size(); i++) {
    for (unsigned sub{0}; sub < max_sub_vec.at(i).second; sub++) {
      ...
      code << "\n\t\treturn "
           << lhs.toString([max_sub_vec, i, sub](exp_unit &e) {
                ...
                for (unsigned j{0}; j < transformed_e.func().func_args.size();
                  ...
                  if (transformed_e.func().func_args.at(j).at(0).type ==
                    ...
                    transformed_e.func().func_args.at(j).at(0) =
                        exp_unit(int(sub));
                    break;
                  }
                }
