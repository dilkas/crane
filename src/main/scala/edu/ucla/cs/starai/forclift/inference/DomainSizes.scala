 ...
 * @note Invariants:
  ...
  */
class DomainSize(val size: Int, val constants: Set[Constant] = Set()) {
...
class DomainSizes(
    ...
    val useExplicitConstants: Boolean = false
) extends MapProxy[Domain, DomainSize] {
  ...
  assume(
    this.forall {
      case (domain: RootDomain, domainSize) => {
        ...
        if (!invsat) {
          println(
            domainSize.size + " < " + domain.staticConstants
              .mkString("[", ",", "]") + " union " + domain.dynamicConstants
              .mkString("[", ",", "]")
          )
        }
        invsat
    ...
    val (domainSize, knownConstants) = this.get(domain) match {
      ...
      case None =>
        throw new IllegalStateException(
          "Unknown domain: %s" format domain.toString
        )
    }
    require(
      !knownConstants.exists(_.toString.startsWith("anon")),
  ...
  def emptyDomains: Option[Set[Domain]] = {
    val empty = self
      .filter {
        case (dom, domSize) => domSize.isEmpty && dom.knownConstants.isEmpty
      }
      .map { case (k, v) => k }
      .toSet
    if (empty.isEmpty) {
...
object DomainSizes {
  ...
  implicit def map2DomainSizes(self: Map[Domain, DomainSize]): DomainSizes =
    new DomainSizes(self)
}
