...
class MLNParser extends JavaTokenParsers with ModelParser {
  ...
  // --- VALS ---
  val domainMap =
    new mutable.HashMap[String, (RootDomain, mutable.HashSet[Constant], Int)]

  /** Map from string to (Subdomain, Parent, Size). Subdomains cannot have
    * constants, only sizes. They can also not be used to define predicates. *
    */
      ...
      case _ => {
        domainMap.get(name) match {
          case Some((domain, constants, current_size)) => {
            domainMap.update(
              name,
              (domain, constants, current_size + with_size)
            )
            domain
          }
          case _ => {
  ...
  def domainSizes: DomainSizes = {
    val roots = domainMap.values.map { case (k, v, s) =>
      (k.asInstanceOf[Domain], DomainSize(s, k.asInstanceOf[Domain], v.toSet))
    }
    val subs = subDomainMap.values.map { case (d, p, s) =>
      if (s == 0) {
        println(
          s"WARNING: Subdomain $d has size zero (if constants are " +
...
//    println(all)
  ...
  def dbDomainSizes: DomainSizes = new DomainSizes(
    dbDomainMap.map { case (k, v) =>
      (
        k.asInstanceOf[Domain],
        DomainSize(v.size, k.asInstanceOf[Domain], v.toSet)
      )
    }.toMap,
    true
  )
  def predicateWeights: PredicateWeights = weightMap.toMap
  def predicateSet: Set[Predicate] =
    predicateMap.values.toSet.filterNot(_ == Predicate.eq)
  ...
  /** */
  lazy val variable: Parser[Var] = (not(reserved) ~> varRe ^^ { name =>
  ...
  /** */
  lazy val term: Parser[Term] = (constant
    | variable
  ...
  /** Parses an atom and returns a Boolean list of those arguments that were
    ...
    */
  lazy val atom: Parser[(Atom, List[Boolean])] = (not(
    reserved
  ) ~> predRe ~ "(" ~ rep1sep(opt("+") ~ term, """\s*,\s*""".r) <~ ")" ^^ {
    case name ~ _ ~ plusarglist => {
      val arglist = plusarglist.map { case p ~ t => t }
      val plusses = plusarglist.map { case p ~ t =>
        p match {
          case None => false
          case _    => true
        }
      ...
      if (isLearnModus || isDatabase) {
        ...
        // constants.
        (arglist zip predicate.domains).foreach { case (a, d) =>
          a match {
            case cons: Constant => {
              if (!d.contains(a.asInstanceOf[Constant])) {
                if (isDatabase) {
                  ...
                  domainMap.get(d.name) match {
                    case Some((_, c, s)) => {
                      printd(
                        "Adding new constant " + cons + " to domain " + d.name
                      )
                      c.add(cons)
                    }
                    case None => {
                      println("Warning: domain not found: %s" format d.name)
                    }
                  }
                }
              }
        ...
        (arglist zip predicate.domains).foreach { case (a, d) =>
          if (
            ...
            throw new IllegalArgumentException(
              "Constant " + a + " is not a constant in domain " + d + "."
            )
          }
        }
      }
    ...
    | not(reserved) ~> predRe ^^ {
      case name => {
        ...
        val predicate = predicateMap.getOrElseUpdate(
          (name, 0),
          new Predicate(Symbol(name), 0)
        )
        (predicate(), Nil)
      }
    }
  ...
  lazy val expatomformula: Parser[Formula] = (atom ^^ {
    case a => {
    ...
    (SP ~ term ~ SP ~ infixOpRe ~ SP ~ term ~ SP ^^ {
      case _ ~ t1 ~ _ ~ op ~ _ ~ t2 ~ _ => {
        op match {
          case "=" => {
            val predicate =
              predicateMap.getOrElseUpdate(("eq", 2), Predicate.eq)
  ...
  /** A formula is considered as an atomic part if it is:
    ...
    */
  lazy val atomicformula: Parser[Formula] = (infixoperator
    | expatomformula
    | "(" ~ SP ~> formula <~ SP ~ ")"
    | "[" ~ SP ~> formula <~ SP ~ "]" ^^ { IndivisibleSubformula(_) }
    | FormulaSyntaxMLN.kwNegation ~ SP ~> expatomformula ^^ { a =>
      NegFormula(a)
    }
    | FormulaSyntaxMLN.kwNegation ~ SP ~ "(" ~ SP ~> formula <~ SP ~ ")" ^^ {
      NegFormula(_)
    }
  ...
  /** Left-associative implication operator
    ...
    *
    * @todo
    *   Double check whether Alchemy is left-associative because logic is
    *   right-associative according to Huth & Ryan 2007.
    *
    * @todo
    *   an exist in an implication without parenthesis is not parsed
    */
  lazy val implformula: Parser[Formula] =
    (disjformula ~ rep1(SPNL ~ kwImplicationRev ~ SPNL ~> disjformula) ^^ {
      ...
      | failure(
        "Expected an implication, disjunction, conjunction, negation or atom."
      ))
  ...
  /** Left-associative equivalence operator
    ...
    *   right-associative according to Huth & Ryan 2007.
    */
  lazy val eqformula: Parser[Formula] = (implformula ~ rep(
    SPNL ~ FormulaSyntaxMLN.kwEquivalence ~ SPNL ~> implformula
  ) ^^ {
    case head ~ Nil => head
    case head ~ list => {
    ...
    | failure(
      "Expected an equivalence, implication, disjunction, conjunction, negation or atom."
    ))

  /** Universal quantifier. */
  lazy val univformula: Parser[Formula] =
    (SP ~ """([Ff]orall|FORALL)""".r ~ SP ~> repsep(
      variable,
      SP ~ "," ~ SP
    ) ~ SP ~ quantformula ^^ { case e ~ _ ~ f =>
      ForallFormula(e, f)
    }
  ...
  lazy val quantformula: Parser[Formula] = (existformula
    | univformula
    | eqformula ^^ {
      case f => {
    ...
    | failure(
      "Expected an quantifier, implication, disjunction, conjunction, negation or atom."
    ))

  /** A formula is an equivalence formula (if present). */
  lazy val formula: Parser[Formula] = (quantformula ^^ {
    case q => {
  ...
  lazy val wformula: Parser[WeightedFormula] =
    (floatingPointNumber ~ """\s+""".r ~ formula ~ opt(
      """\s*,\s*""".r ~> constraints
    ) <~ CRLFEOF ^^ {
  ...
  /** Parse hard or to be learned formulas.
    ...
    */
  lazy val hardlearnformula: Parser[Any] = (formula ~ SP ~ opt(
    """\s*,\s*""".r ~> constraints
  ) ~ opt(".") <~ CRLFEOF ^^ {
    case f ~ _ ~ con_o ~ dot => {
      val con = con_o match {
  ...
  lazy val constraints: Parser[Constraints] = (
    rep1sep((ineqConstr | domConstr), """\s*,\s*""".r) ^^ { matches =>
      {
        ...
        domainMap.get(dom_name) match {
          ...
          case None => {
            subDomainMap.get(dom_name) match {
              case Some((dom, _, _)) => (v, dom)
              case None =>
                throw new IllegalStateException("Domain not known: " + dom_name)
            }
          }
        }
      }
  ...
  /** A domain declaration starts with a domain name followed by an equal sign
    * and a list enclosed by curly braces. Integer domains can be specified with
    * a range (e.g., `3...8`).
    */
  lazy val domainDeclaration: Parser[Domain] =
    (domainRe ~ """\s*=\s*\{\s*""".r ~ repsep(
      constant,
      """\s*,\s*""".r
    ) ~ """\s*\}""".r ^? {
      case name ~ _ ~ _ ~ _ if subDomainMap.contains(name) => {
        throw new IllegalStateException(
          "Subdomain needs to be defined with size ar recursive subdomains: " + name
        )
      }
      case name ~ _ ~ constants ~ _ => {
        val newDomain = new RootDomain(name, constants.toList)
        domains = newDomain :: domains
        domainMap
          .getOrElseUpdate(
            name,
            (newDomain, mutable.HashSet(constants: _*), constants.length)
          )
          ._1
      }
    }
      | domainRe ~ """\s*=\s*""".r ~ wholeNumber ~ """\s*\{\s*\}""".r ^? {
        ...
        case name ~ _ ~ size ~ _ => {
          val newDomain = new RootDomain(name, Nil)
          domains = newDomain :: domains
          domainMap
            .getOrElseUpdate(name, (newDomain, mutable.HashSet(), size.toInt))
            ._1
        }
      }
      | domainRe ~ """\s*=\s*""".r ~ wholeNumber ~ """\s*\{\s*""".r ~ rep1sep(
        constant,
        """\s*,\s*""".r
      ) <~ """\s*\}""".r ^^ {
        case name ~ _ ~ size ~ _ ~ constants => {
          ...
          domainMap
            .getOrElseUpdate(
              name,
              (newDomain, mutable.HashSet(constants: _*), size.toInt)
            )
            ._1
        }
      }
      | domainRe ~ """\s*=\s*\{\s*""".r ~ "[0-9]+".r ~ """\s*,\s*\.\.\.\s*,\s*""".r ~ "[0-9]+".r <~ """\s*\}""".r ^^ {
        case name ~ _ ~ start ~ _ ~ end => {
          val constants = (start.toInt to end.toInt).toList.map { i =>
            Constant(i.toString)
          }
          val newDomain = new RootDomain(name, constants)
          domains = newDomain :: domains
          domainMap
            .getOrElseUpdate(
              name,
      ...
      | domainRe ~ """\s*=\s*\{\s*""".r ~ repsep(
        ...
        case name ~ _ ~ subdomains ~ _ => {
          ...
          val (last_parent, last_name, cnt) = subdomains.toList
            ...
            .foldLeft[(Domain, String, Int)]((new_domain, name, 1)) {
              case ((cur_domain, cur_name, cnt), sd_name) =>
                // TODO: is ComplementDomain used correctly?

                val new_sd =
                  new SubDomain("o_" + cnt, "o_" + sd_name, cur_domain, Set()) {
          ...
          val new_sd = new SubDomain(
            (cnt + 1).toString,
            "o_" + subdomains(subdomains.length - 2),
            last_parent,
            Set()
          ) {
            override lazy val complement = new ComplementDomain(
  ...
  /** Predicate and function declarations
    ...
    */
  lazy val typeDeclaration: Parser[Predicate] = (predRe ~ opt(
    "(" ~ SP ~> repsep(domainRe ~ opt("!"), """\s*,\s*""".r) <~ SP ~ ")"
  ) ~ opt(
    """\s+""".r ~> floatingPointNumber ~ """\s+""".r ~ floatingPointNumber
  ) <~ CRLFEOF ^^ { case predicateName ~ arglist ~ predw =>
    printd(
      "Parsed type declaration: " + predicateName + " with args: " + arglist
    )
    val domainNames = arglist match {
      case Some(domainNames) =>
        domainNames.map {
          case name ~ None => name
          case name ~ exmark => {
            throw new IllegalStateException(
              "Parsing theory failed. The ! operator in a predicate declaration is not yet supported."
            )
            name
          }
        }
    ...
    require(
      !predicateMap.contains((predicateName, arity)),
    ...
    ("#include " ~ SP ~> """[\S]+""".r <~ CRLFEOF ^^ {
      case filename => {
        ...
        try {
          ...
          case e: IOException =>
            throw new Exception(s"Problem with file $filename: " + e.getMessage)
          case e: FileNotFoundException =>
            throw new Exception(s"Can't find file $filename")
        } finally {
          inputfile.close()
        }
      }
  ...
  /** Parse a double forward slash comment line.
    ...
    * since they can appear everywhere and would be too much of an overhead to
    * actually parse.
    */
  lazy val commentLine: Parser[Any] = (commentRe <~ CRLFEOF ^^ {
    case c => {
  ...
  /** Returns a line in the MLN. Note that a line can return multiple lines
    * (e.g., include).
    */
  lazy val anyLine: Parser[List[Any]] = (domainDeclaration ^^ { a =>
    printd("-> Done parsing domain declaration: " + a); List(a)
  }
    | typeDeclaration ^^ { a =>
      printd("-> Done parsing type declaration: " + a); List(a)
    }
    | wformula ^^ { a => printd("-> Done parsing wformula: " + a); List(a) }
    | commentLine ^^ { a =>
      printd("-> Done parsing comment line: " + a); List(a)
    }
    | emptyLine ^^ { a => printd("-> Done parsing empty line"); List(a) }
    | hardlearnformula ^^ { a =>
      printd("-> Done parsing heardlearnformula: " + a); List(a)
    } // hardFormula needs to be after typeDeclaration!
    | includeLine ^^ { a => printd("-> Done parsing include line: " + a); a }
    | failure("Line could not be parsed."))

  lazy val anyLines: Parser[List[Any]] = (rep(anyLine) ^^ { _.flatten })

  lazy val dbLine: Parser[Any] = (SPNL ~> atom <~ SPNL ^^ {
    _ match {
  ...
  lazy val dbLines: Parser[List[Any]] = (rep(dbLine))
  ...
  /** Remove C++ like comment blocks before parse.
    ...
    * can appear everywhere and it would be too much overhead to actually parse.
    *
    * @param input
    *   String containing C++ block comments
    * @return
    *   String with C++ block comments removed
    */
  ...
  /** Add additional weighted formulas that are necessary for the parsed theory.
    *
  ...
  def parseMLN(mlnStr: String): MLN = {
    ...
    mlnlines match {
      case Failure(msg, next) => {
        System.err.println(
          "Error while parsing MLN (line " + next.pos.line + ", column " + next.pos.column + "):\n" + msg + "\n" + next.pos.longString
        )
      ...
      case _ => {
        val wformulas = mlnlines.get.collect { case line: WeightedFormula =>
          line
        }
        val wformulas2 = wformulas.filterNot {
          _.isEmpty
        } ::: additionalWeightedFormulas()
  ...
  def parseMLNWithDB(
      mlnStr: String,
      dbStr: String,
      useExplicitConstants: Boolean = false
  ): MLN = {
    // println("Start MLN parsing")
    // The .mln file contains the MLN for inference
    // val mlnStrNoComments = """(?s).*?/\*.*?\*/.*""".r replaceAllIn ( mlnStr, "" )
    ...
    mlnlines match {
      ...
      case _ => {
        ...
        dblines match {
          case Failure(msg, next) => {
            System.err.println(
              "Error while parsing database (line " + next.pos.line + ", column " + next.pos.column + "):\n" + msg + "\n" + next.pos.longString
            )
  ...
  /** Parse an Alchemy database based on the previously parsed theory. The
    ...
    * evidence in an MLN instance. This MLN should be seen as complementary to
    * the orginal MLN obtained with parseMLN.
    *
    * @param dbStr
